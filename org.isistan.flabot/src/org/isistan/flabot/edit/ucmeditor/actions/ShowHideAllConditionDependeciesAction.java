/**
 * $Id: ShowHideAllConditionDependeciesAction.java,v 1.2 2006/03/21 01:51:56 franco Exp $
 * $Author: franco $
 */
...
public class ShowHideAllConditionDependeciesAction extends SelectionAction {
	...
	public static final String
		SHOW_HIDE_VISUAL_DEPENDENCIES = "SHOW_HIDE_VISUAL_DEPENDENCIES";   //$NON-NLS-1$
	...
	public ShowHideAllConditionDependeciesAction(IWorkbenchPart part) {
		super(part);
		setText(Messages.getString("org.isistan.flabot.edit.ucmeditor.actions.ShowHideAllConditionDependeciesAction.text")); //$NON-NLS-1$
	...
	public Command getCommand() {
		...
		if (isChecked())
			levelDetail = VisualModel.HIGH_DETAIL;
	...
	protected void obtainCommands(CompoundCommand commands, List children, int levelDetail) {
		...
		for (Iterator iter=children.iterator(); iter.hasNext() && retVisual == null;) {
			...
			if (visual.getSemanticModel() instanceof ResponsibilityNode) {
				if (visual.getDetailLevel() != levelDetail)
					commands.add( new SetDetailLevelCommand(visual, levelDetail));
			}
