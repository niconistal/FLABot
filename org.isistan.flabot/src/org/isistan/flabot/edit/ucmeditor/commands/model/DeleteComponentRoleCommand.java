/**
 * $Id: DeleteComponentRoleCommand.java,v 1.2 2006/03/21 01:51:57 franco Exp $
 * $Author: franco $
 */
...
public class DeleteComponentRoleCommand extends CompoundCommand {
	...
	public DeleteComponentRoleCommand(CoreModel coreModel, ComponentRole componentRole)  {
		...
		setLabel(Messages.getString("org.isistan.flabot.edit.ucmeditor.commands.model.DeleteComponentRoleCommand.label")); //$NON-NLS-1$
	}
	...
	public void execute() {
		...
		for (Iterator iter=families.iterator(); iter.hasNext();) {
			...
			for (Iterator iterFes=f.getFamilyElement().iterator(); iterFes.hasNext();) {
				...
				if (fe.getArchitecturalComponent() == componentRole 
						|| fe.getFunctionalComponent()== componentRole)
					add(new DeleteFamilyElementCommand(f, fe));
			}
		}
		super.execute();
