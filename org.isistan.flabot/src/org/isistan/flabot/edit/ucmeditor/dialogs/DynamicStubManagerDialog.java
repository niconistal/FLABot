 ...
 *
 */
public class DynamicStubManagerDialog extends Dialog{
	/**
	 * the dialog's shell
	 */
	...
	public DynamicStubManagerDialog(Shell parent, DynamicStubNode dynamicStubNode, CommandStack commandStack) {
		super(parent, 0);
	...
	public DynamicStubNode open(UCMDiagram diagram, DynamicStubNode dynamicStubNode) {		
...
//		});
	...
	private void handleEdit(Table table) {
		TableItem[] items = table.getSelection();
		if (items.length == 1) {
			ConditionedStub modifiedConditionedStub = (ConditionedStub) items[0].getData();
	...
	public DynamicStubNode getDynamicStub() {
		if (exitValue == SWT.OK) {
...
//			Responsibility resp = (Responsibility)conditionedStub.getResponsibility();
////			StandardEditionDialog<Responsibility> f=getEditionDialog();
//			Command command = openEditResponsibilityDialog(conditionedStub);//f.open(resp);
//			if (command != null) {
