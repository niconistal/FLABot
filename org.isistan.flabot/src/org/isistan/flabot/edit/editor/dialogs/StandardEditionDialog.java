/**
 * $Id: StandardEditionDialog.java,v 1.8 2006/04/03 21:00:32 franco Exp $
 * $Author: franco $
 */
...
public class StandardEditionDialog<T> extends Dialog {
	...
	private void createFinalButtons() {
		...
		buttonProblems.setText(Messages.getString("org.isistan.flabot.edit.editor.problemsButton")); //$NON-NLS-1$
	...
	private List<String> getMessages(EditionItemStatus status) {
		...
		if(statuses.length>0) {
			...
			if(statusMatters(status)) {
				list.add(" - " + status.getDescription()); //$NON-NLS-1$
			}
			return list;
		}
	}
	...
	protected void handleOk() {
		...
		if(canCreateCommand) {
			if(message.length()>0) {
				boolean result=MessageDialog.openConfirm(shell, title,
						Messages.getString("org.isistan.flabot.edit.editor.dialogs.StandardEditionDialog.problemsFoundCanAccept", message)); //$NON-NLS-1$
				if(result) {
			...
			MessageDialog.openError(shell, title,
					Messages.getString("org.isistan.flabot.edit.editor.dialogs.StandardEditionDialog.problemsFoundCanNotAccept", message)); //$NON-NLS-1$
		}
	}
		...
		if(canCreateCommand) {
			if(message.length()>0) {
				MessageDialog.openWarning(shell, title,
						Messages.getString("org.isistan.flabot.edit.editor.dialogs.StandardEditionDialog.problemsFoundCanAcceptWarning", message)); //$NON-NLS-1$
			} else {
				MessageDialog.openInformation(shell, title,
						Messages.getString("org.isistan.flabot.edit.editor.dialogs.StandardEditionDialog.noProblemsFound", message)); //$NON-NLS-1$
			}
		} else {
			MessageDialog.openError(shell, title,
					Messages.getString("org.isistan.flabot.edit.editor.dialogs.StandardEditionDialog.problemsFoundCanNotAccept", message)); //$NON-NLS-1$
