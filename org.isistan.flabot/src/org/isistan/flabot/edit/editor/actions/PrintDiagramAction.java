/**
 * $Id: PrintDiagramAction.java,v 1.5 2006/03/21 02:34:07 franco Exp $
 * $Author: franco $
 */
...
public class PrintDiagramAction extends SelectionAction {
	 ...
	 */
	public PrintDiagramAction(GraphicalViewer viewer,
		...
		setText(Messages.getString("org.isistan.flabot.edit.editor.actions.PrintDiagramAction.text")); //$NON-NLS-1$
		setToolTipText(Messages.getString("org.isistan.flabot.edit.editor.actions.PrintDiagramAction.toolTipText")); //$NON-NLS-1$
	 ...
	 */
	public void run() {
		FlabotMultiPageEditor multiPageEditor =
			(FlabotMultiPageEditor)getWorkbenchPart().getSite().getPage()
			.getActiveEditor();
		IEditorPart activeEditor = multiPageEditor.getActiveEditor();
		String title = "flabot print job"; //$NON-NLS-1$
		if (activeEditor instanceof ComponentEditor) {
			ComponentEditor componentEditor = (ComponentEditor) activeEditor;
			title = Messages.getString("org.isistan.flabot.edit.editor.actions.PrintDiagramAction.componentDiagram") + //$NON-NLS-1$
					componentEditor.getModel().getName();
		}
		else if (activeEditor instanceof UCMEditor) {
			UCMEditor ucmEditor = (UCMEditor) activeEditor;
			title = Messages.getString("org.isistan.flabot.edit.editor.actions.PrintDiagramAction.ucmDiagram") + ucmEditor.getModel().getName(); //$NON-NLS-1$
		}
