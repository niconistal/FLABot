/**
 * $Id: AddPortCommand.java,v 1.2 2006/04/08 05:10:13 dacostae Exp $
 * $Author: dacostae $
 */
...
public class AddPortCommand extends Command {
	...
	public AddPortCommand(VisualModel visualModel, VisualModel parent, Point location) {
		...
		this.parent = parent;
		setLabel(Messages.getString("org.isistan.flabot.edit.componenteditor.commands.AddPortCommand.label")); //$NON-NLS-1$
	}
		...
		for (Iterator iter=parent.getChildren().iterator(); iter.hasNext();) {
			...
			if (visual.getSemanticModel() == port)  {
				MessageDialog dlg = new MessageDialog(
			            ...
			            Messages.getString("org.isistan.flabot.edit.componenteditor.commands.AddPortCommand.label"), //$NON-NLS-1$
			            null,
			            Messages.getString("org.isistan.flabot.edit.componenteditor.commands.AddPortCommand.portAlreadyInComponent"), //$NON-NLS-1$
			            MessageDialog.ERROR,
	...
	public void execute() {	
		...
		if (portModel == null)
			portModel = CoremodelFactory.eINSTANCE.createPortModel();
	...
	protected void updateBounds() {
		...
		for (int i=0; i<children.size(); i++) {
			...
			if ((m.getLocation().getX() == leftOffset && leftSide) ||
			   (m.getLocation().getX() == rightOffset && !leftSide))
				verticalOffset = m.getLocation().getY() + m.getSize().getHeight() + 5;
		}				
