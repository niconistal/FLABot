...
public class ScrollableItemDialog{
	...
	protected Table itemsTable = null;
	protected String title;
	protected String message;
	
	public ScrollableItemDialog(String title,String message)
	{
		this.title=title;
	...
	public void showDialog(List<String> items,ItemProvider itemProvider) {
		try {
			
			for(String s:items)
			{
				itemProvider.decorateError(itemsTable, s);
			}
	...
	private void createPanelItems() {
		...
		itemsTable=new Table(compositeBody,SWT.FILL);
		itemsTable.setHeaderVisible(false);
		itemsTable.setLayoutData(gridData111);
		compositeBody.setBackground( Display.getCurrent().getSystemColor(SWT.COLOR_WHITE));
		
		final TableColumn tc = new TableColumn(itemsTable, SWT.CENTER); // @jve
	...
	private void createCompositeButtons() {
		...
		buttonOK.setText(Messages.getString("org.isistan.flabot.executionmapping.dialogs.ScrollableItemDialog.okButton")); //$NON-NLS-1$
