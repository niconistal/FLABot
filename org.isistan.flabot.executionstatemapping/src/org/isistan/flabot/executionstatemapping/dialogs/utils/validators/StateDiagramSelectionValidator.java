...
public class StateDiagramSelectionValidator  implements ISelectionStatusValidator {
	...
	private static final Status STATUS_ERROR = new Status(IStatus.ERROR,
			...
			Messages.getString("org.isistan.flabot.executionmapping.dialogs.utils.validators.StateDiagramSelectionValidator.mustSelectDiagram"), null); //$NON-NLS-1$
	
	public IStatus validate(Object[] selection) 
	{
		if (selection != null && selection.length == 1) 
		{
			if (((TreeStructuredElement) selection[0]).getType() == TreeType.STATE_DIAGRAM_TYPE)
			{
				return STATUS_OK;
			}
		}
