...
analyzeResponsibility(-1,Family,CurrentScenario,ExecutionScenario,state(CurrentResponsibility,PreResponsibilityId,PreviousScenario,PreviousEvent,State,mapping)):-
       	State = 'Executed',   
       	responsibilityOption(PreResponsibilityId,PreComponentType,_),
		getFamilyToState(CurrentResponsibility,PreResponsibilityId,Family,NewFamily),
		getComponentInstanceSource (NewFamily, PreComponentType, PreInstance),
       	markResponsibility(PreResponsibilityId,PreInstance,state(NewFamily,State,mapping)).    	
